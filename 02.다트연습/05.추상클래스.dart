/********************************************* 
  ...
  * abstract 키워드로 클래스를 선언한다 -> 추상클래스
  abstract는 추상화한...이라는 뜻
  
  abstract class 추상클래스명{
    구현부
    }

  * 추상클래스를 상속받는 것 처럼 연결하는 방법은
 ...
 *********************************************/
 ...
 abstract class Saram{
  ...
  // 5. 말하다메서드 : 내용틀잡기
  String say() => '$name씨는 지금 아무 말도 안한다!';
 ...
 /*********************************************** 
  [ 다트의 오버라이드(override) ]
  -> 기존의 정의된 속성과 메서드를 재정의 하는 것!!(덮어쓰기)
  -> 오버라이드 할때 어노테이션(주석)을 써준다!!
  -> @override 

  참고 : 다트에는 오버로드(overload)가 없다!
  * overload : 같은 이름의 메서드의 파라미터만 그 개수가
  다르게 여러개 만들 수 있는것을 말함 ! 

  ex) $().attr() 등이 같은이름으로 여러개 만들어져있는것
 ...
 ***************************************/
 ...
 // 남지현 배우 클래스 : 사람클래스 일반 상속하기!
 // 일반상속은 부모클래스의 모든 내용을 재정의할 필요없음
 // 단, 구체적인 정의가 없는 메서드는 재정의가 필요하다!!!
 ...
 class SeoJun extends Namja {
  ...
  // 다중상속을 하도록 제한할 수 있다!
  // on Namja 라는 것은 남자클래스를 상속한 경우에만 믹스인사용!
  // -> 결과적으로 남자가수에만 사용가능!!!
...
void testClass (){
  ...
  print(jh.age());
  print(jh.gender());
  print(jh.eat()); // 재정의 하지 않은 메서드는 Saram의 원본 메서드가 출력됨!!
  ...
  // 추상클래스와 여러 믹스인 클래스를 상속받은 준호클래스 인스턴스 생성
  JunHo jho = JunHo('이준호');
  print(jho.age());
  print(jho.gender());
  print(jho.sing());
