...
class _CartDetailState extends State<CartDetail> {
  ...
  Widget _buildSelectButton(int seq, IconData mIcon) {
    ...
    return Container(
      ...
      child: IconButton(
        ...
        onPressed: () {
          // 여기가 매우매우매우 중요함!!
          // 버튼 클릭시 sequenceNum 변수값을 업데이트 함
  ...
  Widget _buildNamePrice() {
    return Padding(
      ...
      child: Row(
        // 메인축 정렬 양쪽 끝(사이간격만 주기)
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          // 1. 상품명 : selectedTit 리스트형 변수값 읽어오기
          Text(
  ...
  Widget _buildDetailIcon(Color mColor) {
    return Padding(
      ...
      // 스택은 아래로부터 위로 겹쳐져서 쌓이는 형태를 말함!
      // 내부에 겹쳐질 위젯은 Positioned() 위젯을 사용함
      // 이 위젭은 웹에서 absolute 포지션과 유사함(top , left 사용가능)
      child: Stack(
        children: [
          Container(
            width: 50,
